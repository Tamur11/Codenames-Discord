...
async def guess(interaction: discord.Interaction, word: str):
    ...
    with GameHandler(DATA_PATH) as g:
        ...
        if no_blue_sm or no_red_sm:
            await interaction.followup.send("Unable to start game: 2 Spymasters required to play.")
            return
        ...
        if current_game.get_turn() == team and current_game.get_guesses() != 0:
            ...
            if 'Correct!' in response:
                await send_image(current_game, interaction, None)
                ...
                if int(current_game.get_guesses()) == -2 or int(current_game.get_guesses()) == -3:
                    message = f"{await get_role_mention(current_game, interaction)} Clue is `{current_clue}`. You have infinite guesses remaining."
                else:
                    current_game.set_guesses(int(current_game.get_guesses()) - 1)
                    message = f"{await get_role_mention(current_game, interaction)} Clue is `{current_clue}`. You have {current_game.get_guesses()} guesses remaining."
...
async def clue(interaction: discord.Interaction, clue: str, number: str):
    with GameHandler(DATA_PATH) as g:
        ...
        await interaction.response.send_message(await get_role_mention(current_game, interaction) + " " + response)
...
async def send_image(current_game: Codenames, interaction, spymaster_color, role_mention=None):
...
async def spymaster_words(current_game: Codenames, interaction, team):
    is_red_sm = get(interaction.guild.roles, name='Red Spymaster')
    is_blue_sm = get(interaction.guild.roles, name='Blue Spymaster')
    spymaster = None
    ...
    if spymaster is not None:
        ...
        await send_image_to_spymaster(current_game, spymaster, team)
        await spymaster.send(f"You are {to_check.name}.")
    else:
        return


async def send_image_to_spymaster(current_game: Codenames, spymaster, team):
    with BytesIO() as image_bin:
...
async def send_endgame_image(current_game: Codenames, interaction):
    with BytesIO() as image_bin:
...
async def get_role_mention(current_game: Codenames, interaction):
    is_blue, is_blue_sm, is_red, is_red_sm = await color_tester(interaction)
    role = None
